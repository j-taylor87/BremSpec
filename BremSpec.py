...
def calculate_median_energy_and_hvl(energy_valid, energy_flux_normalised_filtered):
    """
    Calculate the Half Value Layer (HVL) for the median energy of a filtered energy spectrum,
    including the selection and input of a filter.
    ...
    Returns:
    tuple: A tuple containing the median energy, HVL for the median energy, and selected filter information.
    """
    ...
    if len(indices) > 0:
        ...
        mass_atten_coeff_median = interpolate_mass_atten_coeff([median_energy_at_50pct_auc], energy_Al, mass_atten_coeff_Al)[0]

        # Calculate the HVL for the median energy
        hvl_median = -np.log(0.5) / (mass_atten_coeff_median * density)

        return median_energy_at_50pct_auc, hvl_median
    else:
        # Handle the case where no median is found
        return None  # Return None when median is not found

def calculate_effective_energy_and_hvl(kV, filter_thickness):
    ...
    mass_atten_coeff_eff = -np.log(0.5) / (filter_thickness * density / 1000)
...
if __name__ == "__main__":
    ...
    with col1: # elements in col1will display in the left column
        ...
        show_effective_energy = st.checkbox("Show Effective Beam Energy", value=False)
    ...
    with col2: # elements in col2 will be displayed in the right column
        ...
        median_energy_at_50pct_auc, hvl = calculate_median_energy_and_hvl(energy_valid, energy_flux_normalised_filtered)

        # Call calculate_effective_energy_and_hvl
        energy_eff, hvl_eff = calculate_effective_energy_and_hvl(tube_voltage, filter_1_thickness)
